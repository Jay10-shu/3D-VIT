...
class ViT3(nn.Module):
    ...
    def forward(self, img, mask = None):
        ...
        image_size = img.shape[3]
        frames = img.shape[2]
        num_patches = (image_size // p) * (image_size // p) * (frames // p)
        pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, self.dim)).to(device)
...
# model = ViT(image_size=64,image_patch_size=8,frames=64,frame_patch_size=8,num_classes=2,channels=1,depth=6,dim=1024,heads=8,mlp_dim=2048,emb_dropout=0.1).to("cuda:1")
